...
export function AccountSwitcher({ onSwitch }: AccountSwitcherProps) {
  ...
  const canSwitchToAdmin = userRole === 'developer' || userRole === 'admin';
  ...
  const getAccountAddress = (accountData: AccountData) => formatAccountCityState(accountData);
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          {!isAdmin && currentAccount && (
            <p className="text-[10px] text-[var(--sidebar-muted-foreground)] truncate leading-tight">
              {getAccountAddress(currentAccount) || currentKey}
            </p>
          )}
        </div>
