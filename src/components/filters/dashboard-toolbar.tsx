...
export function DashboardToolbar({
  ...
  const accountLabel = !hasSelectedAccounts
    ? 'All Accounts'
    : singleSelectedAccount
      ? singleSelectedAccount.label
      : `${selectedAccountObjs.length} Accounts`;
  ...
  return (
    <div ref={dropdownRef} className="relative flex items-center gap-2">
      <button
        ...
        className={`inline-flex items-center gap-1.5 h-10 px-3 text-sm rounded-lg border transition-colors ${
          openPanel === 'date'
      ...
      >
        ...
        <span className="sm:hidden">
          {dateRange === 'custom' ? 'Custom' : DATE_RANGE_PRESETS.find(p => p.key === dateRange)?.shortLabel ?? dateRange}
        </span>
      ...
      {showAccountFilter && (
        ...
        >
          {accountIcon}
          <span className="hidden sm:inline max-w-[140px] truncate">{accountLabel}</span>
          <span className="sm:hidden">{hasSelectedAccounts ? 'Filtered' : 'All'}</span>
          {hasSelectedAccounts ? (
            <XMarkIcon
      ...
      {openPanel === 'account' && showAccountFilter && accounts && (
        <div
          className="absolute top-full right-0 mt-2 z-50 glass-dropdown shadow-lg animate-fade-in-up"
          style={{ minWidth: '260px' }}
        >
